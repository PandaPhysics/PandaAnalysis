...
def request_data(xrd_path, first_attempt):
    ...
    if 'scratch' in xrd_path:
        ...
        local_path = xrd_path.replace('root://xrootd.cmsaf.mit.edu/', '/mnt/hadoop/ccccccms')
...
def mvv(good_inputs, output='output.root'):
    good_outputs = ' '.join([input_to_output(x) for x in good_inputs])
    cmd = 'mv ' + good_outputs + ' ' + output
    logger.info(_sname+'.mvv', cmd)
    ret = system(cmd)
    if not ret:
        logger.info(_sname+'.mvv', 'Merging exited with code %i'%ret)
    else:
        logger.error(_sname+'.mvv', 'Merging exited with code %i'%ret)
