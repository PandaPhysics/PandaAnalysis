...
def infer(tt,ifile,payload,features,h5):
	...
	otree = tt.CloneTree()
	#dnnoutput = array.array( 'f', [-99.0 for i in range(5)])
	o_dnnoutput = otree.Branch("dnnoutput" , AddressOf(treestruct,'dnnoutput'), "dnnoutput[5]/F" )
	...
	for i in range(int(tt.GetEntries())):
		...
		for j in range(5):
			treestruct.dnnoutput[j] = model.predict(x)[0,j]
		#print model.predict(x)[0,j]
