...
cuts = {
    'signal'             : tAND(metFilter,tAND(presel,tAND(Ucut,'nLooseMuon==0 && nLoosePhoton==0'))), 
    'singlemuon'         : tAND(metFilter,tAND(presel,tAND(UWcut,'nLoosePhoton==0 && nLooseMuon==1 && nTightMuon==1 && MT(muonPt[0],muonPhi[0],pfmet,pfmetphi)<160'))),
...
#     'dimuon'             : tAND(metFilter,'nLooseElectron==0 && nLoosePhoton==0 && nLooseMuon==2 && nTightMuon>0 && muonPdgId[0]+muonPdgId[1]==0 &&60<Mxx(muonPt[0],muonEta[0],muonPhi[0],0.106,muonPt[1],muonEta[1],muonPhi[1],0.106) && Mxx(muonPt[0],muonEta[0],muonPhi[0],0.106,muonPt[1],muonEta[1],muonPhi[1],0.106)<120 && nTau==0 && jetNMBtags==0 && fabs(calomet-pfmet)/pfUZmag<0.5 && Alt$(DeltaPhi(pfUZphi,jetPhi[0]),99)>0.5 && Alt$(DeltaPhi(pfUZphi,jetPhi[1]),99)>0.5 && Alt$(DeltaPhi(pfUZphi,jetPhi[2]),99)>0.5 && Alt$(DeltaPhi(pfUZphi,jetPhi[3]),99)>0.5 && pfUZmag>250 && GoodLeadJet>2 && LeadJetPt > 100'),
...
weights = {
  'signal'         : '%f*sf_pu*normalizedWeight*1/fabs(mcWeight)*metsf(pfmet)',
  'singlemuon'              : '%f*sf_pu*normalizedWeight*1/fabs(mcWeight)*metsf(pfUWmag)',
  'singleelectron'              :'%f*sf_pu*normalizedWeight*1/fabs(mcWeight)*sf_eleTrig2*Alt$(electronSfTight[0],1)*Alt$(electronSfReco[0],1)',
  ...
  'pho'          : '%f*sf_pu*normalizedWeight*1/fabs(mcWeight)*photrigsf(loosePho1Pt)*sfpho(loosePho1Eta)',
