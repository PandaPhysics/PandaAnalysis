...
void JetOp::do_execute()
{
  ...
  JESLOOP {
	...
	{jw.isLep = true;}
      if (isMatched(matchPhos.get(),0.16,jet.eta,jet.phi))
	jw.isPho = true;
      if (jet.puId < utils.getCorr(cJetLoosePUID, aeta,min(39.99f,pt)))
	jw.isPileupJet = true;
      }
      bool is_tight = false;
      is_tight=((jet.jetId>>1)%2==1); //bit2
      bool is_loose = false;
      is_loose=((jet.jetId>>0)%2==1); //bit2

      if (jw.nominal->isLep || jw.nominal->isPho || jw.nominal->isPileupJet || ((!is_loose && analysis.year == 2016) ||(!is_tight && (analysis.year == 2017 || analysis.year == 2018))))
        continue;
...
void JetFlavorOp::do_execute()
{
  for (auto& jw : (*jesShifts)[0].all) {
    clusteredFlavor(jw);
    if (jw.cleaned_idx >= 0) {
      gt.jetFlav[jw.cleaned_idx] = jw.flavor; 
    }
  }
}
...
void VBFSystemOp::do_execute()
{
  ...
  if (jets.cleaned.size() > idx1) {
    ...
    gt.jet12Mass[shift] = (v0 + v1).M();
    gt.jet12DPhi[shift] = v0.DeltaPhi(v1);
    gt.jet12DEta[shift] = fabs(v0.Eta() - v1.Eta());
  }
}
