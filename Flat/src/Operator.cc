...
void ConfigOp::set_inputBranches()
{
  ...
  if (analysis.genOnly) {
    ...
    bl += {"run", "luminosityBlock", "event","nTrigObj","fixedGridRhoFastjetAll"
           ,"fixedGridRhoFastjetCentralCalo","fixedGridRhoFastjetCentralNeutral"
            ,"genWeight","LHEWeight_originalXWGTUP","LHEPdfWeight","LHEReweightingWeight","LHEScaleWeight","PSWeight","btagWeight"
            ,"Flag","GenPart","Electron","Muon","Tau","Jet","Photon"
            ,"TrigObj"
            ,"SV","GenJet","CaloMET","ChsMET","PuppiMET","RawPuppiMET","TkMET","RawMET","GenMET"
            ,"PV","MET","HLT"
            ,"FatJet","SubJet","CorrT1METJet"
           };
...
void ConfigOp::readData(TString dirPath)
{
  ...
  if      (analysis.year == 2018) {
    ...
    utils.openCorr(cPU,dirPath+"pileup/pileup_BF.root","pileup_scale_BF",1);
    utils.openCorr(cPUUp,dirPath+"pileup/pileup_BF.root","pileup_scale_up",1);
    utils.openCorr(cPUDown,dirPath+"pileup/pileup_BF.root","pileup_scale_down",1);
  } 
  ...
  if (analysis.complicatedLeptons) {
    if (analysis.year==2018) {
      utils.openCorr(cMuLooseID,
                     ...
                     "NUM_LooseID_DEN_TrackerMuons_pt_abseta",2);
      utils.openCorr(cMuMediumID,
                     dirPath+"leptonic/2018_muon_RunABCD_SF_ID.root",
                     "NUM_MediumID_DEN_TrackerMuons_pt_abseta",2);
      utils.openCorr(cMuTightID,
                     dirPath+"leptonic/2018_muon_RunABCD_SF_ID.root",
                     "NUM_TightID_DEN_TrackerMuons_pt_abseta",2);
      ...
      utils.openCorr(cEleVeto,
                     dirPath+"leptonic/2018_ElectronWPVeto_Fall17V2.root",
                     "EGamma_SF2D",2);
      ...
      utils.openCorr(cEleReco,
                     dirPath+"leptonic/egammaEffi.txt_EGM2D_updatedAll.root",
                     "EGamma_SF2D",2);
    } 
    else if (analysis.year==2017) {
      utils.openCorr(cMuLooseID,
      ...
      utils.openCorr(cMuTightIso,
                     ...
                     "NUM_TightRelIso_DEN_TightIDandIPCut_pt_abseta",2);
      utils.openCorr(cEleVeto,
                     dirPath+"leptonic/2017_ElectronWPVeto_Fall17V2.root",
                     "EGamma_SF2D",2);
      utils.openCorr(cEleLoose,
                     dirPath+"leptonic/2017_ElectronLoose.root",
                     "EGamma_SF2D",2);
      utils.openCorr(cEleMedium,
                     dirPath+"leptonic/2017_ElectronMedium.root",
                     "EGamma_SF2D",2);
      utils.openCorr(cEleTight,
                     dirPath+"leptonic/2017_ElectronTight.root",
                     "EGamma_SF2D",2);
      utils.openCorr(cEleMvaWP80,
      ...
      utils.openCorr(cEleReco,
                     ...
                     "EGamma_SF2D",2);
     utils.openCorr(cEleLowReco,
                     dirPath+"leptonic/egammaEffi.txt_EGM2D_runBCDEF_passingRECO_lowEt.root",
                     "EGamma_SF2D",2);
    } 
    else {
      utils.openCorr(cMuLooseID,
  ...
  if(analysis.year==2016){
      ...
      utils.openCorr(cPho,
                 dirPath+"photon/2017_PhotonsMedium.root",
                 "EGamma_SF2D",2);
  }
  else if(analysis.year==2018){
      utils.openCorr(cPho,
                 dirPath+"photon/2018_PhotonsMedium.root",
                 "EGamma_SF2D",2);
  ...
  if(analysis.year==2017){
    utils.openCorr(cTrigEle,
                   dirPath+"trigger_eff/electron_trigger_sf_2017.root",
                   "EGamma_SF2D",2);
    utils.openCorr(cTrigEleDataEff,
                   dirPath+"trigger_eff/electron_trigger_sf_2017.root",
                   "EGamma_EffData2D",2);      
    utils.openCorr(cTrigEleMCEff,
                   dirPath+"trigger_eff/electron_trigger_sf_2017.root",
                   "EGamma_EffMC2D",2);             
    }
  if(analysis.year==2018){
    utils.openCorr(cTrigEle,
                   dirPath+"trigger_eff/electron_trigger_sf_2018.root",
                   "EGamma_SF2D",2);
    utils.openCorr(cTrigEleDataEff,
                   dirPath+"trigger_eff/electron_trigger_sf_2018.root",
                   "EGamma_EffData2D",2);
