  ...
  gt(gt_)
{
    ...
    if (analysis.btagSFs) {
        if (analysis.useDeepCSV) {
          if (analysis.year==2016)
            ...
            calib.reset(new BTagCalibration("DeepCSV", (dirPath+"csv/DeepCSV_94XSF_V5_B_F.csv").Data()));
          ...
          if (analysis.year==2017)
            calib.reset(new BTagCalibration("DeepCSV", (dirPath+"csv/DeepFlavour_94XSF_V4_B_F.csv").Data()));
          else if (analysis.year==2018)
            calib.reset(new BTagCalibration("DeepCSV", (dirPath+"csv/DeepJet_102XSF_V1.csv").Data()));
          else {cout << "input bug" << endl;
                calib.reset(new BTagCalibration("DeepCSV", (dirPath+"csv/DeepCSV_Moriond17_B_H.csv").Data()));}
         }
         else {
          if (analysis.year==2016)
    ...
    if (analysis.btagWeights) {
      ...
      if (analysis.year==2016) {
        ...
        else if (analysis.useDeepCSV){
	      reshaper_calib.reset(new BTagCalibration("DeepCSV", (dirPath+"csv/DeepCSV_94XSF_V5_B_F.csv").Data()));
	}
        else if (analysis.useDeepJet){
            reshaper_calib.reset(new BTagCalibration("DeepFlavour", (dirPath+"csv/DeepFlavour_94XSF_V4_B_F.csv").Data()));
        }
        else 
        ...
        if (analysis.useCMVA) {
          ...
          if (analysis.useDeepCSV)
            ...
            reshaper_calib.reset(new BTagCalibration("DeepFlavour", (dirPath+"csv/DeepJet_102XSF_V1.csv").Data()));
          else
            reshaper_calib.reset(new BTagCalibration("csvv2", (dirPath+"csv/CSVv2_94XSF_V2_B_F.csv").Data()));
        }
      }
