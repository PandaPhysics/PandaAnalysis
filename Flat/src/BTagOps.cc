...
void BTagSFOp::do_execute() 
{
 ...
 //     isIsoJet = true;
    ...
    if (flavor==5)
      ...
      utils.btag->calcSF(bJetM,flavor,eta,pt,eff,btagUncFactor,sf,sfUp,sfDown);
...
void BTagWeightOp::do_execute()
{
  ...
  for (unsigned iShift=0; iShift<GeneralTree::nCsvShifts; iShift++) {
    ...
    for (auto* jw : bcands) {
      ...
      float discr= (analysis.useDeepCSV? jet.btagDeepB :( analysis.useDeepJet? jet.btagDeepFlavB :jet.btagCSVV2));

      int absid=fabs(jet.partonFlavour);
...
/*
      ...
      if      (analysis.useCMVA   ) discr = jet.btagCMVA;
      else if (analysis.useDeepCSV) discr = jet.btagDeepB;
      else                          discr = jet.btagCMVA;
