...
DeepGenOp<GENP>::DeepGenOp(panda::Event& event_,
           Config& cfg_,
           Utils& utils_,
           GeneralTree& gt_,
           int level_) :
  ...
  for (auto* p_ : *genP) {
    ...
    if (p->pt > 0.001 && fabs(p->eta) < 5) {
      if (!analysis.deepGenGrid || (utils.pdgToQ[apdgid] != 0)) { // it's charged, so we have tracking
        finalStates.emplace_back(temP4.Px(), temP4.Py(), temP4.Pz(), temP4.E());
...
//          const GENP *parent = p;
          ...
          while (p->genPartIdxMother>=0) {
            auto* parent = dynamic_cast<const GENP*>(genP->at(p->genPartIdxMother));
...
//      const GENP* parent = gen;
      while (gen->genPartIdxMother>=0) {
        auto* parent = dynamic_cast<const GENP*>(genP->at(gen->genPartIdxMother));
...
//    const GENP *parent = gen;
    ...
    for (const auto* child_ : *genP) {
      ...
      if(child->genPartIdxMother>=0){
        auto* parent = dynamic_cast<const GENP*>(genP->at(child->genPartIdxMother));
        if(parent == gen) nopass=false;
      }
      if(nopass) continue;
/*      
...
*/
  ...
  TLorentzVector vTmp;
  for (auto *p : partons) {
    vTmp.SetPtEtaPhiM(p->pt, p->eta, p->phi, p->mass);
    vPartonSum += vTmp;
  }
...
template class DeepGenOp<panda::GenPart>;
