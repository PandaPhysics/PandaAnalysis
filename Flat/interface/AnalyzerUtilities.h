...
namespace pa { 
  inline bool hard(const panda::GenPart& p) 
  { 
    if ((p.statusFlags>>7)%2==1) return true;
    else return false;
//    return p.testFlag(panda::GenPart::kIsHardProcess);
  }

  bool hasChild(const panda::GenPart& p, const std::vector<panda::Particle*>& genP, panda::Event& event, bool isHard=false);
  bool isAncestor(const panda::GenPart& child, const panda::GenPart& ancestor, panda::Event& event );
  ...
  inline const panda::GenPart* pToGPtr(const panda::Particle* p)
  {
    auto* g = dynamic_cast<const panda::GenPart*>(p);
    if (g == nullptr) {
      logger.error("pToGPtr",Form("Trying to cast a non-GenPart at %p", p));
      throw std::runtime_error("");
    }
    return g;
  }

  inline const panda::GenPart& pToGRef(const panda::Particle* p)
  {
    // I think this is what I want - the underlying data type really is a GenPart
    return *pToGPtr(p);
  }
  ...
  VPseudoJet convertPFCands(const std::vector<const panda::PFParticle*> &incoll, bool puppi, double minPt=0.001);
  VPseudoJet convertPFCands(const panda::RefVector<panda::PFParticle> &incoll, bool puppi, double minPt=0.001);
...
////////////////////////////////////////////////////////////////////////////////////
  ...
  struct JetWrapper {
    ...
    const panda::FatJet* Fatbase;
    const JetWrapper* nominal{nullptr}; 

    JetWrapper(float pt_, const panda::Jet& j): pt(pt_), base(&j) { }
    JetWrapper(float pt_, const panda::FatJet& j): pt(pt_), Fatbase(&j) { }

    const panda::Jet& get_base() const { return *base; }
    const panda::FatJet& get_Fatbase() const { return *Fatbase; }
    float scale() const { return pt / base->pt; }
    float fat_scale() const{ return pt/Fatbase->pt;}
    void p4(TLorentzVector& v) const {
      v.SetPtEtaPhiM(pt, base->eta, base->phi, base->mass); 
    }
