...
/*
  ...
  public:
    FatJetReclusterOp(panda::Event& event_,
                       Config& cfg_,
                       Utils& utils_,
                       T& gt_,
                       int level_=0) :
...
/*      substructure(analysis.recalcECF ? 
      ...
      } else if (analysis.year==2018) { // DeepCSV subjet b-tagging in 2018
...
//    std::unique_ptr<SubRunner> substructure{nullptr};
  ...
  public:
    FatJetMatchingOp(panda::Event& event_,
                      Config& cfg_,
                      Utils& utils_,
                      GeneralTree& gt_,
                      int level_=0) :
...
//    std::shared_ptr<const std::vector<panda::FatJet*>> fjPtrs{nullptr};
    ...
    std::map<const panda::GenPart*,float> genObjects; // gen particle -> pt
    const panda::GenPart* matchGen(double eta, double phi, double r2, int pdgid=0) const;
  };
}
/*
  class HRTagOp : public HROp {
  public:
    HRTagOp(panda::Event& event_,
              Config& cfg_,
              Utils& utils_,
              HeavyResTree& gt_,
              int level_=0) :
